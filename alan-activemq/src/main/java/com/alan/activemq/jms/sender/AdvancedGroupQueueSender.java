package com.alan.activemq.jms.sender;

import com.alan.activemq.utils.StringUtils;
...
public class AdvancedGroupQueueSender implements SmartLifecycle {
    ...
    private ActiveMQDestination defaultQueue = null;

    private Map<String, JmsTemplate> jmsTemplateMap = new HashMap<String, JmsTemplate>();

    private Map<String, PooledConnectionFactory> PooledConnectionFactoryMap =
            new HashMap<String, PooledConnectionFactory>();
    ...
    public void sendMsg(final String message, String subQueueName, final String systemId)
            throws JmsException {
        ...
        MessageCreator messageCreator = new MessageCreator() {
            ...
            public Message createMessage(Session session) throws JMSException {
                ...
                if (!StringUtils.isBlank(systemId)) {
                    txtmsg.setStringProperty("systemcode", systemId);
    ...
    public void send(String subName, MessageCreator messageCreator) throws JmsException {
        ...
        for (int k = 0; k < maxConnection * tokenFactor * brokerURLs.size(); k++) {
            try {
                ...
                throw new UncategorizedJmsException(
                        "blockingQueue.take() interrupted, " + e.getMessage());
            } catch (JmsException e) {
                token.free();

                // 打印堆栈信息，以方便定位问题
                String errMsg = ExceptionUtils.getFullStackTrace(e);
                logger.warn("failed to send message to broker {" + token.brokerUrl + "}," + errMsg);
        ...
        if (success == false) {
            throw new UncategorizedJmsException(
                    "failed send message to  anyone of brokers, maybe brokers are shutdown");
        }
    }
    ...
    public void start() {
        ...
        if (isTopic()) {
            defaultQueue = new ActiveMQTopic(groupQueueName + jointMark + "default");
        } else {
        ...
        for (String url : brokerURLs) {
            PooledConnectionFactory pooledConnectionFactory =
                    new PooledConnectionFactory(new ActiveMQConnectionFactory(url));
